...
export const Select = <T,>({
  ...
  return (
    <Field className={twMerge("relative w-full p-3", className)}>
      <div className="flex">
        {label && (
          <Label className="my-auto px-3 whitespace-nowrap text-sm font-medium leading-6 text-gray-900 dark:text-gray-200">
            {label}
          </Label>
        )}
        <Listbox value={value} onChange={onChange} disabled={disabled}>
          <div className="relative">
            ...
            <Transition
              ...
              leaveTo="opacity-0"
            >
              <ListboxOptions className="absolute z-10 mt-1 max-h-60 overflow-auto rounded-md bg-white dark:bg-gray-800 py-1 text-base shadow-lg ring-1 ring-black dark:ring-gray-700 ring-opacity-5 focus:outline-none sm:text-sm">
                {children}
              </ListboxOptions>
            </Transition>
          </div>
